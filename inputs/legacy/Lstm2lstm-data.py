...
# %%
import copy
withPhonrepsR = copy.deepcopy(withPhonreps)
withPhonrepsL = copy.deepcopy(withPhonreps)
withOrthrepsR = copy.deepcopy(withOrthreps)
withOrthrepsL = copy.deepcopy(withOrthreps)

# %%
# first create the dictionary with the right pads for the phon
for word in pool:
    padlen = cfg['maxphon']-len(withPhonreps[word])
    p = withPhonrepsR[word]

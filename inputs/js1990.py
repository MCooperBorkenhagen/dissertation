...
# get chateau and js1990 experimental lists for training:
js = pd.read_csv('raw/jared_etal_1990_e1.csv').word.tolist()
...
# %%
jsc_words = list(set(ca + cb + cc + js))
...
# missing words from js1990
with open('./raw/jared_etal_1990_missing.json') as m:
    missing4 = json.load(m)

missing1.update(missing2)
missing1.update(missing3)
missing1.update(missing4)

with open('raw/missing-words-js-all.json', 'w') as f:
...
#%%
js = d(words, outliers=outliers, cmudict_supplement='raw/missing-words-js-all.json', maxorth=MAXORTH, maxphon=MAXPHON, minorth=MINORTH, minphon=MINPHON, maxsyll=MAXSYLL, justify='left', terminals=True, onehot=False, orthpad=9, frequency=frequencies)


#%%
# save the traindata to a pickle file with extension *.traindata
save(js.traindata, 'js1990/js.traindata')

with open('js1990/phonreps-with-terminals.json', 'w') as t:
    json.dump(js.phonreps, t)



#%% # syllabics

with open('js1990/syllabics.csv', 'w') as s:
    s.write('word,foot,heart,core,leg,head,body,phon,onset,anchor,nucleus,oncleus,coda,rime,nsyll,contour,vowels\n')

    for orthform, phonform in js.cmudict.items():

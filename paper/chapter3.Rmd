...
Early on in training, increases in word frequency are associated with lower errors, as shown in Figure XX. This association is lower in both computational models than in the naming accuracy data for the training words from @Balota2007, but in all three we see a relationship between frequency and errors. This comparison demonstrates that by including frequency as an aspect of training we see an impact on performance, regardless of architecture.

```{r, fig.cap="A descriptive relationship between word frequency (scaled) and word-level error is shown for human behavioral data (left), a feedforward network architecture (middle), and the time-varying/ LSTM architecture (right). Here error is calculated as how close the produced output was to the target output relative to all other targets in the training set. In all three cases, an increase in frequency is associated with an increase in errors."}
...
descriptives_model = mono %>%
  ...
  filter(stage %in% c('Early', 'Late')) %>% 
  group_by(model, stage) %>% 
  summarise(MEAN = mean(nearest_phon_rank),
            SD = sd(nearest_phon_rank))
...
descriptives_elp = mono %>% 
  ...
  filter(stage == 'Late' & model == 'LSTM') %>% #arbitrary because values repeat
  filter(train_test == 'train') %>% 
...
Furthermore, the effect of frequency fades as learning progresses; at the end of training even the least frequent items show a high level of accuracy. This is due to the fact that structural complexity (here in orthography, phonology and the mappings between them) bears most heavily on performance when experiences with that structure are limited, but with time a learner can overcome complexity with repeated learning opportunities.

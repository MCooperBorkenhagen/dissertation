...
mono %>% 
  ...
  select(word, model, acc, consistency) %>% 
  rbind(d_elp) %>% 
  mutate(model = case_when(model == 'LSTM' ~ 'Time-varying',
                           TRUE ~ model)) %>% 
...
tabledata$Predictor = c('Intercept', 'Learning stage', 'Consistency', 'Frequency', 'Learn stg x consist', 'Learn stg x freq', 'Consistency x frequency', 'Lrn stg x consist x freq')

rownames(tabledata) = NULL
```

```{r, stageFrequencyConsistencyModelRender} 



HEADER = c('', 'b', 'SE', 't', 'p', '95% CI')
names(tabledata) = HEADER
names(tabledata)[2] = cell_spec(names(tabledata)[2], italic = T)

...
In order to validate the model using words with more than one syllable, two analyses are presented. First, a set of relevant predictors taken from @Yap2009 are examined. Then, experiments that examine the effects of consistency in multisyllabic words are reported, namely those from @Chateau2003 and @Jared1999.
...
It should be noted that there is a theoretical assumption that underlies this orthographic segment as described in @Taft1992 that runs contrary to ideas about learning espoused in this dissertation. @Taft1992 articulates ideas about syllabic structure in orthography, which doesn't square with the theoretical assumption that orthographic units aren't processed in syllabic segments. It also assumes that orthography contains homologous structure to that of spoken language syllabic structure. The computational model developed here and the theory of cross-modal perceptual processing conveys a different assumption: that printed words are processed in a continuous fashion (over some grain size) and not syllable by syllable, just like spoken language. The ideas about the syllabic structure of orthography in @Taft1992 are indeed contested [@Chateau1990; @Jared1990], and the empirical findings about the nature of the basic orthographic syllabic structure have been challenged experimentally [@Seidenberg1997]. More generally, the extent to which syllables (like other subcomponents of words) function independently in as perceptual subcomponents has been debated as well [@Seidenberg1987].

Nonetheless, the experiments in @Chateau2003 are useful because their stimuli account for the fact that orthographic segments can be associated with spoken language segments at grain sizes larger than what traditionally might be considered a syllable boundary in orthography and that quasi-regularity [@Plaut1996] can be conceptualized at a level of grain that can in words longer than one syllable. The findings here 

The patterns of means in all experiments in @Chateau2003 are seen in the model data here. Figure XX shows the data from the three experiments on top (acquired via personal communication with Debra Jared) and the corresponding data from the computational model on bottom using error data during training as the outcome variable.

```{r, cj2003expsCombined, echo=FALSE}

```


## Variable effects over time
It should be noted again that effects of expected word-level characteristics on error shift over time in the computational model's performance. This variability isn't typically reported in analogous computation work related to reading development, but it bears a bit of observation and discussion here. Here are the patterns of means for the interaction between consistency and frequency across training using the @Chateau2003 data (Experiment 3).

```{r, cj2003AcrossTraining, echo=FALSE}
...
However, if we standardize for our outcome variable (mean squared error here) within each training cycle, and average across them you see the expected pattern emerge (Figure XX).
...
multi %>% 
  ...
  geom_line() +
  scale_color_manual(values = COLORS) +
  labs(x = 'Frequency', y = 'Mean squared error (standardized)', color = 'Consistency') +
  theme_apa()
...
It should be noted that the consistency effects from @Jared1990 weren't successfully replicated by the model.
...
```{r, js1990ModelData, echo=FALSE, fig.align='center', fig.height=3, fig.width=5}

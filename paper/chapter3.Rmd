...
source('chapter3_multisyllabic_data.R')
source('plot-specs.R')
```
...
## Frequency
More experience with a word leads to more robust representation of the word, which in turn enhances the ability to act on it in some form. The most common proxy for experience in the reading literature is the frequency of a word's occurrence in a set of texts or other corpus. Words that are more frequently encountered tend to be easier to read, resulting in general in faster reading times and fewer errors [@Balota2004; @Forster1973; @Stanovich1978]. Connectionist models of reading aloud have established the effect of experience in a number of studies, where experience is implemented by proxy as a frequency-influenced procedure during learning (XX).
...
Words with more syllables are associated with greater errors at the end of training, both using training mode (with phonological inputs provided during the calculation of loss) and in test mode, where only the start of word representation is provided to initiate an output.
...
The types of mistakes made by the model vary, with `r nrow(multi_mistakes)` words out of `r nrow(multi_testmode)` (or `r nrow(multi_mistakes)/nrow(multi_testmode)`% of all words) pronounced incorrectly. Table XX shows a sample of the mistakes made for words of different lengths.
...
In order to validate the model using words with more than one syllable, two analyses are presented. First, a set of relevant predictors taken from @Yap2009 are examined. Then, experiments that examine the effects of consistency in multisyllabic words are reported, namely those from @Chateau2003.
...
predictors =  c('Frequency', 'N syllables', 'Phon length', 'Orth length', 'Orth N', 'Phon N')

tabledata = summarows(tmp, c('freq', 'nsyll',  'phonlen', 'orthlen', 'orth_lev', 'phon_lev'), newnames = predictors, na.rm = T, manuscript = T, notes = 'Frequency values provided from HAL, which is one of the frequency sources native to the ELP data, thus permitting easy direct comparisons with the model implementation here. Phonological length was calculated directly from the phoneme transcriptions used in the model (from the Natural Language Toolkit implementation of CMU dictionary.)')
...
While the correlations among word variables and processing difficulty among the model and ELP datasets are comparable, let's look more closely at two of the variables: number of syllables and phonological length. These two word characteristics are of particular interest in the computational model for different reasons. First, considering the number of syllables. In the model proposed, syllables aren't explicitly represented in the model architecture or input representations. There is a debate in the word recognition literature about whether something like _syllabic processing_ is a necessary component of word recognition with, of course, particular attention paid to longer (multisyllabic) words in determining this level of representation in the reading system. @Jared1990 posited that the processing of multisyllabic words can't be reduced to a syllable-by-syllable process. Their data from Experiment 4 show that when words are delivered one syllable at a time, the magnitude of the effect of inconsistency on naming increases, demonstrating that the context facilitates their processing. This is expected in a theory where the processing of syllables is simply part of a broader processing system that associates speech from print by exploiting the statistical (contextual) dependencies in orthography (i.e., a connectionist theory). If syllables were "assembled", you would expect to see left-to-right syllabic processing without regard to the (orthographic context of) the subsequent syllable in experimentation.


The phonological length of a word is a potentially important characteristic because of the nature of the architecture. Given that the way this model is configured it relies heavily on the phonological processing of words throughout the learning process, it is worthwhile to consider the possibility that the architecure overrepresents phonology during processing; phonology might play too strong a role in the architecture relative to human performance.
...
@Jared1990 sought to extend the findings of @Glushko1979 to multisyllabic words. Their first experiment examined the effect of consistency on naming using a very similar paradigm to @Glushko1979 (and @Taraban1987) with stimuli where the consistency varied systematically across syllables (specifically either the first syllable or last syllable of a set of multisyllabic words). Because previous implementations 

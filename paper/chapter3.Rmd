...
header = c('Predictor', '*r* (Mod)', '*r* (ELP)')
...
tabledata$`*r* (Mod)` = sig_level(cortests(tmp, 'mse', corvars, 'estimate'), cortests(tmp, 'mse', corvars, 'p.value'))
tabledata$`*r* (ELP)` = sig_level(cortests(tmp, 'elp_rt', corvars, 'estimate'), cortests(tmp, 'elp_rt', corvars, 'p.value'))


apa_table(tabledata, caption = 'Correlations between Word Variables and Processing Difficulty (MSE for Model and RT for Behavioral Data)', note = "Correlations provided as Pearson's *r*. Bolded coefficients are significant at *p* < .001.")

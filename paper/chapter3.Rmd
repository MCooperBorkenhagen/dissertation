...
## Training details
In order to avoid padded input and output representations for training, words were assembled for learning based on phonological length for reasons discussed previously. Batches consisted of words of the same phonological length, where the orthographic length of words within a batch could vary but were accommodated with masked timesteps on input (i.e., timesteps that are skipped rather than empty). This is due to the fact that words that share the same number of phonemes don't necessarily share the same number of letters. The constraint associated with batches requires a specification about the order of learning that is unique to the architecture: how should the order of learning trials be determined given the fact that batches need to be arranged with words that have the same phonological length. A number of configurations are possible, ranging from batching all words of a single length together to training on a batch size of one. The former was chosen for efficiency reasons; training times are reduced greatly if larger batches are used. Investigating the role of batch size on learning, and the behavioral correlates in humans would no doubt be a worthwhile avenue of future research bu is outside the scope of the work being done here.

Given these considerations, batches (i.e., words defined by their phonological length) were sampled using a weighted sampling procedure within each epoch. Within epoch, batches with shorter words were more likely to come before those with longer words, while (by definition) all words within the training set were included in training within each epoch. This procedure was used to avoid a strict ordering of batches based on phonological length in training.

...
## Frequency
More experience with a word leads to more robust representation of the word, which in turn enhances the ability to act on it in some form. The most common proxy for experience in the reading literature is the frequency of a word's occurrence in a set of texts. Words that are more frequently encountered tend to be easier to read, resulting in general in faster reading times and fewer errors [@Balota2004; @Forster1973; @Stanovich1978]. Connectionist models of reading aloud have established the effect of experience in a number of studies, where experience is implemented by proxy as a frequency-influenced procedure during learning [@Sibley2010; @Plaut1996]. Word frequencies were used in order to scale gradients during training for the time-varying model. One issue is whether the time-varying architecture is sensitive to experience implemented in this way, and whether or not this sensitivity resembles that of other computational architectures and human behavioral data.

In order to examine the effect of frequency on accuracy, mean squared error for the time-varying and feedforward models were generated halfway through training. These are reported alongside both latency and accuracy values for the training pool items taken from @Balota2007. All values were standardized in order to share a y-axis, this involved arranging the accuracy scores such that increases in the values represented increases in processing difficulty (i.e., mirroring the distribution of response times). As a result, these values can be interpreted as "processing difficulty". Also, frequencies have been log transformed. Increases in word frequency are associated with a reduction in processing difficulty, as shown in Figure 4. This relationship is lower in both computational models than in the data from @Balota2007, but in all four we see a common pattern.
...
\newpage
Furthermore, as with the effect of frequency fades as learning progresses; by the end of training this effect is attenuated substantially such that accuracy is consistently high among most words. With enough experience, processing difficulty is minimized. In table 5 we see this in the relationship between raw error (as mean square error) plotted at two points in training: once early on (at 36 epochs) and again late in training (at 72 epochs). Errors become low even for the least frequent words.

```{r, freqeffectEarlyLate, echo=FALSE, fig.height=3, fig.cap= 'The effect of word frequency early and late in training on the time-varying network differs such that with ample experience errors on infrequent words are minimized and the overall effect of frequency on error is reduced. Raw errors are plotted for words (points) using mean squared error.' }

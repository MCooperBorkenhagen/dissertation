...
#%%
learner = Learner.Learner(orth_features, phon_features, phonreps=phonreps, orthreps=orthreps, traindata=train, modelname='monosyllabic', hidden=400, devices=False)
#%%
...
for i in range(1, C+1):
    cycle_id = 'pre-'+str(i)
    learner.fitcycle(batch_size=32, cycles=3, probs=probs, K=K, evaluate=True, cycle_id=cycle_id) 
#%%
learner.model.save('../outputs/mono/lstm/monosyllabic-model-pre')
...
"""Done with advanced training"""
#%%
learner.model.save('../outputs/mono/lstm/monosyllabic-model-advanced')
...
#%%
np.savetxt('../outputs/mono/lstm/posttest-outputs-targets-distance-matrix.csv', d_targets_by_outputs)
# %% create the same for the less advanced lstm model, just in case those are useful:
premodel = keras.models.load_model('../outputs/mono/lstm/monosyllabic-model-pre')
...
for data in train.values():
    for i, word in enumerate(data['wordlist']):
        y = data['phonEOS'][i].flatten()
        y_hat = premodel.predict([reshape(data['orth'][i]), reshape(data['phonSOS'][i])]).flatten()
        train_outputs[row][0:y_hat.shape[0]] = y_hat
        train_targets[row][0:y.shape[0]] = y
        l1.append(word)
        print(word, 'done', '...the', str(row)+'th word')

...
"""Acquiring data for the Learner learning to read multisyllabic words that include the experimental lists from Chateau & Jared (2003)"""
...
test = load('data/chateau/test.traindata')
...
K = pd.read_csv('data/chateau/K.txt', header=None)[0].tolist()[0]

learner = Learner.Learner(orth_features, phon_features, phonreps=phonreps, orthreps=orthreps, traindata=train, testdata=test, loss='binary_crossentropy', modelname='chateau', hidden=400, devices=False)
# %%
gentests = 99
cycles = 9
for j in range(1, 12): # stop on the 11th cycle
    cycle = j*cycles
    learner.fitcycle(cycles=cycles, epochs=1, probs=probs, K=K, outpath='../outputs/chateau', evaluate=True, cycle_id=str(cycle))
    print(cycle, 'done')
    if cycle == gentests:
        learner.model.save('../outputs/chateau/model-epoch{}'.format(cycle))
        model_json = learner.model.to_json()
        with open('../outputs/chateau/model-epoch{}.json'.format(cycle), 'w') as json_f:
            json_f.write(model_json)

        learner.model.save_weights('../outputs/chateau/model-epoch{}-weights.h5'.format(cycle))
        ...
        with open('../outputs/chateau/generalization-epoch{}.csv'.format(cycle), 'w') as gt:
            gt.write(colnames)

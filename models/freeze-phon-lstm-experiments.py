...
such conditions. These trials freeze weights only on the phonological LSTM"""
...
#
learner = Learner.Learner(orth_features, phon_features, phonreps=phonreps, orthreps=orthreps, traindata=train, testdata=test, loss='binary_crossentropy', modelname='freezephon-lstm', hidden=400, devices=False)
...
for i in range(1, C+1):
    ...
    learner.fitcycle(batch_size=32, cycles=CYCLES, probs=probs, K=K, evaluate=True, cycle_id=cycle_id, outpath='../outputs/freezephon-lstm/0', verbose=0) 
...
for i in range(1, C+1):
    ...
    learner.fitcycle(batch_size=32, cycles=CYCLES, probs=probs, K=K, evaluate=True, cycle_id=cycle_id, outpath='../outputs/freezephon-lstm/18', verbose=0) 
...
for i in range(1, C+1):
    ...
    learner.fitcycle(batch_size=32, cycles=CYCLES, probs=probs, K=K, evaluate=True, cycle_id=cycle_id, outpath='../outputs/freezephon-lstm/36', verbose=0) 
...
for i in range(1, C+1):
    ...
    learner.fitcycle(batch_size=32, cycles=CYCLES, probs=probs, K=K, evaluate=True, cycle_id=cycle_id, outpath='../outputs/freezephon-lstm/54', verbose=0) 
keras.backend.clear_session()
...
for i in range(1, C+1):
    cycle = i*CYCLES
    cycle_id = str(cycle)    
    learner.fitcycle(batch_size=32, cycles=CYCLES, probs=probs, K=K, evaluate=True, cycle_id=cycle_id, outpath='../outputs/freezephon-lstm/never', verbose=0) 

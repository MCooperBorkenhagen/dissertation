...
"""Acquiring data for the Learner learning to read multisyllabic words"""
...
test = load('data/multi-test.traindata')
train = load('data/multi-train.traindata')
...
K = pd.read_csv('data/multi-K.txt', header=None)[0].tolist()[0]
#%%
learner = Learner.Learner(orth_features, phon_features, phonreps=phonreps, orthreps=orthreps, traindata=train, testdata=test, loss='binary_crossentropy', modelname='multi', hidden=400, devices=False)
# %%
...
for j in range(1, 12): # stop on the 11th cycle
    cycle = j*cycles
    learner.fitcycle(cycles=cycles, epochs=1, probs=probs, K=K, outpath='../outputs/multi', evaluate=True, cycle_id=str(cycle))
    learner.model.save('../outputs/multi/multi-model-epoch{}'.format(cycle))
    model_json = learner.model.to_json()
    with open('../outputs/multi/model-epoch{}.json'.format(cycle), 'w') as json_f:
        json_f.write(model_json)

    learner.model.save_weights('../outputs/multi/model-epoch{}-weights.h5'.format(cycle))
    print(cycle, 'done')
    if cycle == gentests:
        ...
        with open('../outputs/multi/multi-generalization-epoch{}.csv'.format(cycle), 'w') as gt:
            gt.write(colnames)
...
#%%
np.savetxt('../outputs/multi/test-outputs.csv', test_outputs)
np.savetxt('../outputs/multi/train-outputs.csv', train_outputs)
...
#%%
np.savetxt('../outputs/multi/outputs-targets-distance-matrix-epoch{}.csv'.format(cycle), d_targets_by_outputs)
...
with open('../outputs/multi/order.csv', 'w') as f:
